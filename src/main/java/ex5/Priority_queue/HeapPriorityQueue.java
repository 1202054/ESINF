...
public class HeapPriorityQueue<K,V> extends AbstractPriorityQueue<K,V> {
  ...
  public boolean isPriorityQueue(ArrayList<Entry<K,V>> lstEntry){

    for (int i = 0; i < (lstEntry.size()-2)/2; i++) {

      if (left(i) < lstEntry.size() && compare(lstEntry.get(left(i)),lstEntry.get(i))<0)
        return false;
      if (right(i) < lstEntry.size() && compare(lstEntry.get(right(i)),lstEntry.get(i))<0)
        return false;
    }
    return true;
  }
